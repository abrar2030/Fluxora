...
def lambda_handler(event, context):
    ...
    try:
        ...
        report_key = f"compliance-reports/{environment}/{timestamp}_compliance_report.csv"
        ...
        try:
            ...
            for result in compliance_details['EvaluationResults']:
                compliance_data = {
                    ...
                    'config_rule_invoked_time': result['ConfigRuleInvokedTime'].isoformat()
                }
...
def generate_summary_report(report_data):
    ...
    if summary['total_resources_evaluated'] > 0:
        summary['overall_compliance_percentage'] = round(
            (summary['compliant_resources'] / summary['total_resources_evaluated']) * 100, 2
        )
    else:
