...
from fluxora.models.train import (load_data, objective, prepare_training_data,
...
def test_train_lstm_model(
    mock_dropout, mock_dense, mock_lstm, mock_sequential, prepared_data
):
...
def test_objective_xgboost(
    ...
    assert (
        mock_trial.suggest_float.call_count >= 3
    )  # learning_rate, subsample, colsample_bytree
    assert (
        mock_trial.suggest_int.call_count >= 3
...
def test_objective_lstm(
    ...
    # Create mock Sequential model
    mock_model = MagicMock()
    mock_model.predict.return_value = np.expand_dims(
        y_val, axis=1
    )  # Reshape to match LSTM output
    mock_sequential.return_value = mock_model
