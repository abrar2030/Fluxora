...
def generate_mock_data(start_date: str = "2023-01-01", end_date: str = "2023-01-31", num_meters: int = 5) -> pd.DataFrame:
    """
    ...
    for i, meter_id in enumerate(meter_ids):
        ...
        df.loc[mask, 'target'] = base_consumption[mask] + (i * 5) + np.random.normal(0, 2, size=mask.sum())
        
    # Ensure all targets are positive
    df['target'] = df['target'].clip(lower=0)
    
    # Add context features (e.g., temperature, humidity)
    df['temperature'] = 15 + 10 * np.sin(2 * np.pi * df['timestamp'].dt.dayofyear / 365) + np.random.normal(0, 1, size=len(df))
    df['humidity'] = 60 + 15 * np.cos(2 * np.pi * df['timestamp'].dt.dayofyear / 365) + np.random.normal(0, 2, size=len(df))
