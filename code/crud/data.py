...
def get_data_records(db: Session, user_id: int, skip: int = 0, limit: int = 100):
    return db.query(EnergyData).filter(EnergyData.user_id == user_id).offset(skip).limit(limit).all()
...
def get_data_by_time_range(db: Session, user_id: int, start_time: datetime, end_time: datetime):
